...
// i18n
let addToCartText = formatMessage({ id: 'cart:add_to_cart_text', default: 'Add to Cart', description: 'The button text for adding an item to the cart.' });
let removeFromCartText = formatMessage({ id: 'cart:remove_from_cart_text', default: 'Remove from Cart', description: 'The button text for removing an item from the cart.' });
let removeFromCartTextVariant = formatMessage({ id: 'cart:remove_from_cart_text_variant', default: 'Remove', description: 'The button text for removing an item from the cart.' });
let checkoutButtonText = formatMessage({ id: 'cart:checkout_text', default: 'Checkout', description: 'The button text for checking out the cart items, proceeding to payment.' });
let orText = formatMessage({ id: 'concept:or', default: 'or', description: 'The word \'or\' when used in a choice. i.e., \'apples or oranges\'' });
let cartAsyncButtonText = formatMessage({ id: 'cart:async_btn_text', default: 'Working...', description: 'The text displayed while a cart item is being added to the cart, while waiting for a server response.' });
let subtotalLabel = formatMessage({ id: 'cart:subtotal_label', default: 'Subtotal:', description: 'The text before the amount of currency that the sum of the items in the cart amount to.' });
let currencySymbol = formatMessage({ id: 'concept:currencySymbol', default: '$', description: 'The currency symbol used.' });
let cartEmptyText = formatMessage({ id: 'cart:cart_empty', default: 'Your cart is empty', description: 'The text to display when there are no items inside of the cart.' });
let shoppingCartText = formatMessage({ id: 'cart:cart_title', default: 'Your Cart', description: 'The text displayed at the top of the cart indicating that it is the user\'s shopping cart.' });
...
export class AddToCartButton extends React.Component {
  ...
  addToCart() {
    ...
    cart.addItem( product_id ).then(() => {
      ...
      let lineId = cart.getCartItems().filter(el => el.attrs.product_id === product_id);
      ...
      that.setState({ inCart: true, loading: false, lineId });
    });
  }
    ...
    if ( this.state.inCart ) {
      ...
      return (
        <div className={cx( 'add-to-cart-actions' )}>
          ...
          <em className={cx( 'in-between-button' )}>&ndash; {orText}
            &ndash;</em>
...
let cartDetails = {
  ...
  'quantity': formatMessage({ id: 'cart:quantity', default: 'Quantity', description: 'The heading for the cart\'s column \'quantity\', indicating that the column contains the number of items' }),
  'item': formatMessage({ id: 'cart:item', default: 'Item', description: 'The heading for the cart\'s column \'item\', indicating the name of the product.' }),
  'price': formatMessage({ id: 'cart:price', default: 'Price', description: 'The heading for the cart\'s column \'price\', indicating the cost of the product.' }),
};
  ...
  render() {
    ...
    if ( this.state.ready ) {
      ...
      if ( cartItemCount > 0 ) {
        ...
        subtotal = <div className={cx( 'subtotal' )}>
          <strong>{subtotalLabel}</strong>
          {currencySymbol}{cart.cart.subtotal}</div>;
      } else {
    ...
    <table className={cx( 'cart-items' )}>
      ...
      <tbody>
        {props.cartItems.map(( lineItem, i ) => {
          return ( <CartItem key={i} {...lineItem} lineId={lineItem.id} index={i}/> );
        })}
      </tbody>
    </table>
  );
...
export class CartItem extends React.Component {
  ...
  render() {
    let src = this.props.attrs.image.src
      ? this.props.attrs.image.src
      : client.NO_IMAGE_URI;
    return (
      <tr className={cx( 'cart-item' )}>
        ...
        <td className={cx( 'cart-item-quantity' )}>
          <input type='number' value={this.props.attrs.quantity} onChange={this.changeQuantity}/>
        </td>
