...
const collectionRoutes = data.keys.map(( name, i ) => {
  ...
  if ( data.collections[name].products ) {
    ...
    products = products.map(( product, _i ) => {
      ...
      return <Route path={`/collections/${ name }/items/${ handle }`} component={collectionItemPages[name][handle ]} key={_i}/>;
    });

  }

  return (
    <Route path={`/collections/${ name }`} title={title( `${ name } Collection` )} component={collectionPages[name]} key={i}>
      {products}
    </Route>
  );
});
